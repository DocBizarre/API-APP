...
# ─── Résolution des dépendances (database.py / amelioration_generator.py) ─────────
...
for _c in _CANDIDATS:
    if (_c / "database.py").is_file() and (_c / "amelioration_generator.py").is_file():
...
try:
    ...
    _mb.showerror(
        ...
        "Impossible de trouver « database.py » et « amelioration_generator.py ».\n\n"
...
#   2. Fichier  config_amelioration.txt  contenant une ligne  db=<chemin>  (prio 2)
...
def _resoudre_base_partagee():
    ...
    if env and Path(env).is_file():
        return Path(env)
    # 2. Fichier de config
    cfg = _HERE / "config_amelioration.txt"
    if cfg.is_file():
